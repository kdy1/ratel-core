use ratel::ast::expression::{ArrowBody, ArrowExpression};
use ratel::ast::statement::ReturnStatement;
use ratel::ast::{Block, ExpressionNode, Function, Name, NodeList, OptionalName};
...
impl<'ast> Visitor<'ast> for TransformArrow<'ast> {
    fn on_arrow_expression(
        &mut self,
        node: &ArrowExpression<'ast>,
        ...
        let body = match node.body {
            ...
            ArrowBody::Expression(expr) => {
                let ret = self
                    .ctx
                    .alloc_as_loc(&expr, ReturnStatement { value: Some(expr) });

                self.ctx.alloc_as_loc(
                    &ret,
                    Block {
